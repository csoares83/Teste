...
public class MultiplePnT2 extends WriteApp{
	...
	protected void parseArgs(CommandLine cli, String[] args)
		...
		minMsgDelay = Long.parseLong(cli.getOptionValue(minMsgDelayOption,"0")); // default: 10 minutes
		...
		capInfra=Integer.parseInt(cli.getOptionValue(capInfraOption,"-1"));
                System.out.println("==========================================================================");
		System.out.println("Max Message Delay "+maxMsgDelay);
                infra_only  = cli.hasOption(infraOnlyOption);
                System.out.println("Utiliza Infraestrutura: "+infra_only);
		if ( ! infra_only ){

			ds_oracle = cli.hasOption(dsOption);
			cc_oracle = cli.hasOption(ccOracleOption);
                        System.out.println("Utiliza ds_oracle: "+ds_oracle);
                        System.out.println("Utiliza cc_oracle: "+cc_oracle);
			if ( ! (ds_oracle  || cc_oracle)){
				
				if (cli.hasOption(sprayAndWaitOption)){
                                        System.out.println("Utiliza Spray-And-Wait : true");
...
//		}
		...
		TransferTrace infra_transfers = (TransferTrace)_store.newTrace("infra_transfers", TransferTrace.type, force);
                MessageEventLogger msgLogger = new MessageEventLogger(messages.getWriter(links.maxUpdateInterval()));
		BufferEventLogger bufferLogger = new BufferEventLogger(buffers.getWriter(links.maxUpdateInterval()));
		TransferEventLogger adhocTransferLogger = new TransferEventLogger(adhoc_transfers.getWriter(links.maxUpdateInterval()));
		TransferEventLogger infraTransferLogger = new TransferEventLogger(infra_transfers.getWriter(links.maxUpdateInterval()));
		//		
		...
		// filename for test(
		String filename1=report_folder+numPushed+"-delay-"+"-msg-";

		new InfectionDistributionReport(filename1, min_time,max_time,messages, buffers, presence ).exec();
		...
		//String filename3=report_folder+"rx-delay-node-"+"random"+"claiton";

		//new NodeReceptionDelayReport(filename3, min_time,max_time,messages, buffers ,presence).exec();
